#!/usr/bin/env python3
"""Transcribe the curriculum and a produced schedule into a Writ instance.
...
feeding.
    ...
    ./to_writ.py schedule.json -o timetable.writ
"""
import argparse
import json
import os
...
# Members of the enumerated types timetable.lib.writ declares. Names are global
# across the loaded universe and may not be redeclared (Writ §7), so an entity
...
class Names:
    """Every entity name emitted, kept distinct — as Writ requires."""
...
def capacity(S, fresh):
    """Seat counts as a ladder of classes: Writ compares by walking, not by <."""
    if len(S.sizes) > 4:
        sys.exit(f"{len(S.sizes)} distinct capacities, but `fits` in "
                 f"timetable.lib.writ walks a ladder of 4 — extend the form or "
...
def main():
    ...
    ap.add_argument("-o", "--out", default="timetable.writ")
    ap.add_argument("--lib", default="timetable.lib.writ")
    args = ap.parse_args()
    ...
    body = [f"; GENERATED by to_writ.py from {os.path.basename(args.curriculum)} "
